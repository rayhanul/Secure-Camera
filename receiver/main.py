...
class TSNReceiver:
    """
    Receives compressed JSON payloads over UDP/TSN.
...
class TransReIDProcessor:
    ...
    def load_model(self, model_path=None, config_file=None):
        ...
        try:
            ...
            cfg.MODEL.PRETRAIN_PATH = "/app/TransReID/weights/jx_vit_base_p16_224-80ecf9dd.pth"
...
class C2Processor:
    """Main TSN receiver processor with Weaviate integration"""
    ...
    def normalize_incoming_data(self, data: Dict) -> Dict:
        """
        Convert incoming sender objects into the format expected by process_detection().
        """
    ...
    def save_simple_results(self, data: Dict, reid_results: List[Dict]):
        try:
            ...
            if not self.results_saver:
                print("⚠️ Results saver not initialized. Enable save_results in arguments.")
                return
...
def parse_args():
    parser = argparse.ArgumentParser(description="TSN Receiver with Weaviate ReID")
    ...
    parser.add_argument(
        ...
        type=str,
        default="/app/TransReID/weights/vit_transreid_market.pth",
    )
    parser.add_argument(
